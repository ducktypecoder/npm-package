...
module.exports = function startProject(args) {
  ...
  git.clone(projectIdentifier, projectSlug).then(() => {
    files.changeIntoDirectory(projectSlug);
    // NOTE: for some reason, this works when calling simple-git directly
    // but when using hte promisified-wrapper module
